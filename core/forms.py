...
class VisiteForm(forms.ModelForm):
    ...
    def __init__(self, *args, user=None, **kwargs):
        ...
        self.fields['service_visite'].queryset = Service.objects.filter(is_archived=False)
...
class UserRegistrationForm(forms.ModelForm):
    ...
    def __init__(self, *args, current_user=None, **kwargs):
        ...
        if self.instance.pk:
            ...
            # Si c'est un superuser
            if self.instance.is_superuser:
                self.fields['porte_actuelle'].required = False
                
                # Masquer si :
                # - L'utilisateur connecté n'est pas 'admin_secours'
                # - OU l'utilisateur à modifier est 'admin_secours' lui-même (pas de modif propre)
                hide_fields = True
                if current_user and current_user.username == 'admin_secours' and self.instance.username != 'admin_secours':
                    hide_fields = False
                    
                if hide_fields:
                    if 'porte_actuelle' in self.fields:
                        del self.fields['porte_actuelle']
